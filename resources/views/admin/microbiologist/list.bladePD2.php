                                    ...
                                    <table id="example1" class="table table-striped table-bordered responsive col-xlg-12" cellspacing="0" width="100%">
                                        <thead>
                                            <tr>
											  ...
											  <th>View / Edit Result</th>
												<!--<th>Interim</th>-->
											  <th>Request for Retest</th>
											  <th>Add Another Test</th>
											  <th>Result to Nikshay</th>
                                            ...
                                            @foreach ($data['sample'] as $key=> $samples)
										     ...
										     @if(($samples->print_15a==1)&&($samples->sent_to_nikshay==0))
                                                  <tr id="<?php echo $key; ?>" class="sel">
													...
													<td  <?php echo $data['services_col_color['.$samples->enroll.']']=='Y'?'bgcolor="#ccffcc"':""; ?>
                          >
                          <?php echo $data['test_requested['.$samples->enroll.']']; ?>
                            
                          </td>
													<td>{{$samples->test_reason}}</td>
													 <!-- <td><a href={{$samples->url}}>{{$samples->service_name}}</a></td> -->
                                                    <td>{{$samples->service==15?$samples->stag:$samples->service_name}}
													   <a target="_blank" href="{{url('interimview/'.$samples->sampleID)}}" class="btn btn-info btn-sm">Draft Result</a>
                                                    </td>
													
													 <td>
                                                    <button type="button" onclick="openCbnaatForm1({{$samples->enroll}},'{{$samples->sample}}','{{$samples->service}}','{{$samples->samples}}','{{$samples->service==15?$samples->stag:'NULL'}}')" class="btn btn-info btn-sm resultbtn" >View</button>
                                                    
													<!-- $samples->service==22?4:5-->
                                                    @if($samples->service_name!='BWM')
														@if($samples->service_name=='LJ - DST Inoculation' || $samples->service_name=='LJ - DST 2nd Line')
                                                    <a href="javascript:void(0);" onclick="editResultForm('{{$samples->sampleID}}',{{$samples->enroll}},'{{$samples->samples}}','{{$samples->service}}','','{{$samples->druglist}}','{{$samples->drug_ids}}')">Edit
												    <?php //echo '<pre>'; print_r($samples->druglist);?> <?php //echo '<pre>'; print_r($samples->drug_ids);?></a>
												     @else
														 <!----other than LC DST,LJDST--------->
												    <a href="javascript:void(0);" onclick="editResultForm('{{$samples->sampleID}}',{{$samples->enroll}},'{{$samples->samples}}','{{$samples->service}}','{{$samples->stag}}','{{$samples->druglist}}','{{$samples->drug_ids}}')">Edit</a>
													  @endif  
                                                    @endif
                                                    </td>
													                           <td>
                                                      <?php $tag= (!empty($samples->stag))? $samples->stag : '' ?>
                                                    <!--button type="button" onclick="openCbnaatFormretest({{$samples->enroll}},'{{$samples->sample}}','{{$samples->service}}','{{$samples->samples}}')" class="btn btn-info btn-sm resultbtn" >Retest</button-->
					                                <button type="button" onclick="openCbnaatFormRetest( '{{$samples->enroll}}','{{$samples->samples}}','{{$samples->service}}','{{$samples->sampleID}}','{{$samples->bwm_status}}','{{$samples->no_sample}}', '{{ $samples->reg_by }}','{{($samples->service==14||$samples->service==15)?$samples->stag:'NULL'}}',{{ $samples->rec_flag }});" class="btn btn-info btn-sm resultbtn" >Retest</button>
                                                    </td>

													                           <td>
                                                    <button type="button" onclick="openCbnaatFormAddtest({{$samples->enroll}},'{{$samples->sample}}','{{$samples->service}}','{{$samples->samples}}',{{$samples->rec_flag}})" class="btn btn-info btn-sm resultbtn addTestBtn" >Add Test</button>
                                                    </td>
													<td>
                                                    <button type="button" onclick="openCbnaatFormNikshay({{$samples->tslID}},'{{$samples->enroll}}','{{$samples->sample}}','{{$samples->service}}','{{$samples->samples}}','{{$samples->enroll_l}}','{{$samples->service==15?$samples->stag:''}}',<?php echo $data['reqServ_service_id['.$samples->enroll.']']; ?>)" class="btn btn-info btn-sm resultbtn" >Submit</button>
                                                    </td>
													
													
													<td>{{$samples->facility_id}}</td>
													<td>{{$samples->sample_type}} / 
													    {{$samples->no_of_samples}} </td>
                                                    <td><?php echo date('d/m/Y', strtotime($samples->date)); ?></td>
                                                    <td><a class="detail_modal" style="color:#1E88E5; cursor:pointer; font-size:12px;" onclick="getdetailsform(<?php echo $id=$samples->sampleID; ?>,<?php echo $enrolls_id=$samples->enroll; ?>,'<?php echo $sample_label=$samples->samples; ?>')">Show Status</a></td>
                                                   
                                                    <!-- <td>{{$samples->m_detail}}</td>
                                                    <td>{{$samples->m_remark}}</td> -->
                                                    <!--<td>
													
                                                    <button type="button" onclick="openCbnaatFormDrug({{$samples->enroll}},'{{$samples->sample}}','{{$samples->service}}','{{$samples->samples}}',
                                                    '{{$samples->drug_ids}}')" class="btn btn-info btn-sm resultbtn" >Choose Drugs</button>
                                                    ...
                                                    <button type="button" onclick="openCbnaatForm( '{{$samples->enroll}}','{{$samples->samples}}','{{$samples->service}}',{{$samples->sampleID}},'{{$samples->bwm_status}}','{{$samples->no_sample}}', '{{ $samples->reg_by }}');" class="btn btn-info btn-sm resultbtn" >Submit</button>
...
function openForm15AGenerate(enroll_id, sample_ids, service, sample, bwm_status, no, reg_by){
 ...
 function openCbnaatFormNikshay(tServiceLogID,enroll_id,sample_id,service_id,sample_no,enroll_no,tag=null,reqServ_service_id){    
