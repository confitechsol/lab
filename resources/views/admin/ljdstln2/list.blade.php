            ...
            <div class="container-fluid">
              <div class="row page-titles">
                  <div class="col-md-5 col-8 align-self-center">
                      <h3 class="text-themecolor m-b-0 m-t-0">LJ DST 2nd Line</h3>

                  </div>
                  <div class="col-md-7 col-4 align-self-center">
                    <form action="{{ url('/ljdstsecond/print') }}" method="post" >
              ...
              @include('admin/ljdstln2/readingpopup')
                                      ...
                                      <tbody>
                                        ...
                                        <tr>
                                          ...
                                          <td>
                                            ...
                                            <a href="#" class="btn btn-info btn-sm" onclick="openReadingForm('{{$samples->enroll_id}}','{{$samples->sample_id}}','{{$samples->service_log_id}}',4,'{{$samples->druglist}}','{{$samples->drug_ids}}')">Add Reading</a>
                                            @else
                                            <a href="#" class="btn  btn-sm" onclick="openReading({{$samples->enroll_id}},4)">View Details</a>

                                            @endif

                                          </td>
                                          <td>
										  <?php //echo 'samples->status'.$samples->status; echo 'samples->week_no'.$samples->week_no; ?>
                                          ...
                                          <td>
										  <?php //echo 'samples->ljdst_status'.$samples->ljdst_status; ?>
                                            @if($samples->status==1 && $samples->week_no==6 && $samples->ljdst_status!=0)
            ...
            <div class="modal-dialog">
              ...
              <div class="modal-content">
                <div class="modal-header">
                  ...
                  <h4 class="modal-title">LJ DST 2st Line</h4>
                </div>
                 <form class="form-horizontal form-material" action="{{ url('/lj_dst_ln2') }}" method="post" enctype='multipart/form-data' id="cbnaat_result">
                          @if(count($errors))
...
function openReadingForm(enroll_id, sample_id, service_log_id, week, drugs, drugs_id){
   console.log(drugs);
...
function openReading(lj_dst_reading_id,week){
  $.ajax({
    type: "GET",
    url: "{{url('lj_dst_ln2/detail/')}}"+"/"+lj_dst_reading_id+"/"+week,
    success: function(obj){
      ...
      $('#readingdetpopupDiv2').modal('toggle');
    },
    dataType: "json"
  });
}
  ...
  today = dd + '-' + mm + '-' + yyyy;
    $('#exampl').DataTable({
        ...
        buttons: [
            {
                extend: 'excelHtml5',
                title: 'LIMS_lj_dst_2nd_line_'+today+''
            }
        ],
        "order": [[ 1, "desc" ]]
    });
