            ...
            @php( $chart = [
                ...
                'data'=> [
                    ...
                    'datasets' => [
                        ...
                        [
                            'label' => 'No of Shipment on Transit from other Lab',
                            'backgroundColor' => '#5588ff',
                            'data' => $shipment_transit_from_other_lab->values(),
                        ],

                        [
                            'label' => 'No of Shipments Received from other Lab',
                            'backgroundColor' => '#ffee00',
                            'data' => $shipment_received_from_other_lab->values(),
                        ],

                        [
                            'label' => 'No. of Shipment Transferred to Other Lab',
                            'backgroundColor' => '#55cc00',
                            'data' => $shipment_transferred_to_other_lab->values(),
                        ],
    ...
    <div class="card mb-3">
        <div class="card-block">
            <table class="table table-sm table-striped">
                ...
                <tr>
                    <th>No of Shipment Received from other Lab</th>
                    @foreach( $sentTos as $sentTo )
                        <th class="text-center">{{ $shipment_received_from_other_lab[ $sentTo->name ] }}</th>
                    @endforeach
                </tr>
                <tr>
                    <th>No of Shipment on Transit from other Lab</th>
                    @foreach( $sentTos as $sentTo )
                        <th class="text-center">{{ $shipment_transit_from_other_lab[ $sentTo->name ] }}</th>
                    @endforeach
                </tr>
                <tr>
                    <th>No. of Shipment Transferred to Other Lab</th>
                    @foreach( $sentTos as $sentTo )
                        <th class="text-center">{{ $shipment_transferred_to_other_lab[ $sentTo->name ] }}</th>
                    @endforeach
                </tr>
                </tbody>
            </table>
