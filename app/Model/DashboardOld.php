...
class Dashboard extends Model
{
    public static function dashboard_list(){
            ...
            $data['service']['sample']=DB::select('select s.id as service_id, s.name, ifnull(count(m.id),0) as cnt from m_services s left join t_service_log as m on m.service_id = s.id where s.id in (1,2,3,4,8,12,14,15,17,18,20,21,22,23,25) and m.status in (0,1,2) group by m.service_id');

            $data['service']['sample_tested']=DB::select('select s.id as service_id, s.name, ifnull(count(m.id),0) as cnt from m_services s left join t_service_log as m on m.service_id = s.id where s.id in (1,2,3,4,8,12,14,15,17,18,20,21,22,23,25) and m.status = 1 group by m.service_id');

            $data['service']['sample_review']=DB::select('select s.id as service_id, s.name, ifnull(count(m.id),0) as cnt from m_services s left join t_service_log as m on m.service_id = s.id where s.id in (1,2,3,4,8,12,14,15,17,18,20,21,22,23,25) and m.status = 2 group by m.service_id');

            $data['service']['sample_result']=DB::select('select s.id as service_id, s.name, ifnull(count(m.id),0) as cnt from m_services s left join t_service_log as m on m.service_id = s.id where s.id in (1,2,3,4,8,12,14,15,17,18,20,21,22,23,25) and m.status = 0 group by m.service_id');
